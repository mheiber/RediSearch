...
inline int IR_HasNext(void *ctx) {
  ...
  // ir->buf->offset, ir->header.size > ir->buf->offset);
...
inline int IR_GenericRead(IndexReader *ir, t_docId *docId, float *freq,
                          u_char *flags, VarintVector *offsets,
                          t_docId expectedDocId) {
  if (!IR_HasNext(ir)) {
    return INDEXREAD_EOF;
  }
...
int IR_Read(void *ctx, IndexHit *e) {
  ...
  int rc = IR_GenericRead(ir, &e->docId, &freq, &e->flags, offsets, 0);

  // add tf-idf score of the entry to the hit
  if (rc == INDEXREAD_OK) {
    // LG_DEBUG("docId %d Flags 0x%x, field mask 0x%x, intersection: %x",
    // e->docId, e->flags, ir->fieldMask, e->flags & ir->fieldMask);
...
@return INDEXREAD_OK if found, INDEXREAD_NOTFOUND if not found, INDEXREAD_EOF if
at EOF
*/
int IR_SkipTo(void *ctx, u_int32_t docId, IndexHit *hit) {
  ...
  if (ent != NULL || ir->skipIdx == NULL || ir->skipIdx->len == 0 ||
    ...
    if (ent != NULL && ent->docId > ir->lastId) {
      IR_Seek(ir, ent->offset, ent->docId);
    }
    
    // the seek table hit exactly!
    if (docId && ir->lastId == docId) {
...
size_t IW_Len(IndexWriter *w) { return BufferLen(w->bw.buf); }
...
void IW_GenericWrite(IndexWriter *w, t_docId docId, float freq, u_char flags,
  ...
  LG_DEBUG("docId %d, flags %x, Score %f, quantized score %d", docId, flags,
           freq, quantizedScore);
...
inline t_docId IR_LastDocId(void *ctx) { return ((IndexReader *)ctx)->lastId; }
...
int UI_Read(void *ctx, IndexHit *hit) {
  ...
  do {
    ...
    *hit = ui->currentHits[minIdx];
    hit->type = H_UNION;
    ui->minDocId = ui->currentHits[minIdx].docId + 1;
    return INDEXREAD_OK;
...
int UI_HasNext(void *ctx) {
  ...
  for (int i = 0; i < u->num; i++) {
    if (u->its[i] && u->its[i]->HasNext(u->its[i]->ctx)) {
      return 1;
    }
  }
  return 0;
...
int UI_SkipTo(void *ctx, u_int32_t docId, IndexHit *hit) {
  ...
  for (int i = 0; i < ui->num; i++) {
    // iterators for non existing words will be null
    if (ui->its[i] == NULL) continue;

    // no need to skip
    if (ui->currentHits[i].docId < docId || docId == 0) {
      if ((rc = ui->its[i]->SkipTo(ui->its[i]->ctx, docId,
                                   &ui->currentHits[i])) == INDEXREAD_EOF) {
        continue;
      }
    }
  ...
  do {
    ...
    for (i = 0; i < ic->num; i++) {
      ...
      if (h->docId != ic->lastDocId || ic->lastDocId == 0) {
        if (ic->its[i] == NULL ||
            (rc = ic->its[i]->SkipTo(ic->its[i]->ctx, ic->lastDocId, h)) ==
                INDEXREAD_EOF) {
          return INDEXREAD_EOF;
        }
      }
      ...
      if (rc == INDEXREAD_OK) {
        ...
        ic->its[i]->Read(ic->its[i]->ctx, &ic->currentHits[i]);
      }
    }
    ...
    if (nh == ic->num) {
      ...
      if (hit != NULL) {
        ...
        for (int i = 0; i < nh; i++) {
          ...
          while (hit->numOffsetVecs < MAX_INTERSECT_WORDS &&
                 n < hh->numOffsetVecs) {
      ...
      if (ic->its[0]->Read(ic->its[0]->ctx, &ic->currentHits[0]) ==
          INDEXREAD_EOF) {
        // if we're at the end we don't want to return EOF right now,
        // but advancing docId makes sure we'll read the first iterator again in
        // the next round
        ic->lastDocId++;
      } else {
      ...
      // LG_DEBUG("Flags %x, field mask %x, intersection: %x", hit->flags,
      // ic->fieldMask, hit->flags & ic->fieldMask);
